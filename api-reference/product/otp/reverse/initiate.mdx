...
<Info>
    Set `service` to `"sms"` or `"imessage"`. It defaults to `"sms"` when omitted. iMessage requires an active iMessage-capable leased number and an iMessage entitlement. [Learn more about Reverse OTP with iMessage](/api-reference/product/imessage/verify).
</Info>
