...
Reverse OTP verifies a phone number by having the user send a code to you. Initiate a session with `contiguity.otp.reverse.initiate()`:
...
Set `service` to `"sms"` or `"imessage"`. It defaults to `"sms"` when omitted. iMessage requires an active iMessage-capable leased number and an iMessage entitlement.

Use the returned `otp_id` to check or cancel the session:
...
See the [Reverse OTP API reference](/api-reference/product/otp/reverse/initiate) for all parameters and response fields, or read more about [Reverse OTP with iMessage](/api-reference/product/imessage/verify).
